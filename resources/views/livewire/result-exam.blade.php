        ...
        <div class="row">
            <form >
                <div class="form-group">
                    {{-- @dump($userAnswer) --}}
                    <p>Result (<span style="color:green"> {{ $currect }} </span><span style="color:red"> {{ $wrong }} </span>/ {{ $total }}) , number Q {{ $number +1  }}  </p>
                    <label for="question">Question: {{ $question['text'] }}</label>
                    @forelse ($question['options'] as $key => $option)
                        {{-- @dump($option['opt']) --}}
                        <div class="custom-radio">
                            <input type="radio" id="{{ $number.$key }}" disabled @if ($key == $question['answer'] ) checked  @endif >
                            <label for="{{ $number.$key }}" @if ($question['answer'] != $question["userAnswer"]&&$key == $question["userAnswer"]) style="background-color: red; color: white;"
                            @endif >{{ $option }}</label>
                        </div>
        ...
        <script>
            document.addEventListener('livewire:load', () => {
                Livewire.on('afterDomUpdate', () => {
                    history.replaceState(null, null, '?q=' + @this.number);
                });
            });
        </script>
    @endscript
